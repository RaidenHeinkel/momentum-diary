...
if st.session_state.current_page == "calendar":
    ...
    for week in cal:
        ...
        for i, day in enumerate(week):
            if day != 0:
                ...
                if cols_days[i].button(f"🔹{day}" if has_d else str(day), key=f"btn_{d_str}", type="primary" if is_selected else "secondary", use_container_width=True):
    ...
    if st.session_state.previous_date != d_str or content_key not in st.session_state:
        ...
        st.session_state[content_key] = st.session_state.local_updates.get(d_str, df[df['date'] == d_str]['content'].values[0] if not df[df['date'] == d_str].empty else "")
    ...
    if col_list.button("📊 一覧", use_container_width=True):
        save_current_diary_if_changed(); st.session_state.current_page = "list"; st.rerun()

# =====================================================================
# 画面２：一覧（リアルタイム検索）
# =====================================================================
elif st.session_state.current_page == "list":
    st.markdown("""<style>.stButton > button { height: auto !important; min-height: 4.5rem; padding: 0.6rem 0.8rem !important; display: flex !important; flex-direction: column !important; justify-content: flex-start !important; align-items: flex-start !important; text-align: left !important; width: 100% !important; } .stButton > button * { text-align: left !important; justify-content: flex-start !important; align-items: flex-start !important; margin: 0 !important; width: 100% !important; display: block !important; white-space: pre-wrap !important; } .stButton > button p { -webkit-line-clamp: 5 !important; overflow: hidden !important; font-size: 0.85rem !important; } div[data-testid="column"] .stButton > button { min-height: auto !important; padding: 0.4rem 0.8rem !important; display: inline-flex !important; } </style>""", unsafe_allow_html=True)
    ...
    for d, c in st.session_state.local_updates.items():
        ...
        elif c.strip() != "": df_list = pd.concat([df_list, pd.DataFrame([{"date": d, "content": c}])], ignore_index=True)
    df_list = df_list[df_list['content'].str.strip() != ''].sort_values(by='date', ascending=False)

    col_back, col_title = st.columns([1.3, 4.7])
    if col_back.button("⬅️ 戻る", use_container_width=True): st.session_state.current_page = "calendar"; st.rerun()
    
    search = st.text_input("", placeholder="🔍 キーワード検索...", key="search", label_visibility="collapsed")
    if search: df_list = df_list[df_list['content'].str.contains(search, case=False, na=False) | df_list['date'].str.contains(search, na=False)]
    
    col_title.markdown(f"<p style='margin:0; padding-top:6px; font-size:1.1rem; font-weight:bold;'>📊 一覧（{len(df_list)}件）</p>", unsafe_allow_html=True)
    with st.container(height=520):
        for idx, row in df_list.iterrows():
            if st.button(f"📅 {row['date']}\n{row['content'][:300]}", key=f"l_{idx}", use_container_width=True):
                st.session_state.edit_date = row['date']; st.session_state.edit_header = row.get('header', ''); st.session_state.current_page = "edit"; st.rerun()
...
# =====================================================================
    ...
    ek = f"edit_content_{ed}"
    if ek not in st.session_state: st.session_state[ek] = st.session_state.local_updates.get(ed, get_data(SHEET_URL)[get_data(SHEET_URL)['date'] == ed]['content'].values[0] if not get_data(SHEET_URL)[get_data(SHEET_URL)['date'] == ed].empty else "")
    if st.button("⬅️ 戻る"): st.session_state.current_page = "list"; st.rerun()
    up_c = st.text_area("", key=ek, height=360)
    if st.button("💾 保存", type="primary", use_container_width=True):
        if save_diary(ed, st.session_state.edit_header, up_c): st.success("保存完了")
